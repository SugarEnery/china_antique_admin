<template>
    <div class="container messageboard">
        ...
        <el-table :data="tableData" v-loading="listLoading"  border stripe style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="article_title" label="文章标题" align="center" >
            </el-table-column>
            <el-table-column prop="type" label="区分类型" align="center" >
              <template slot-scope="scope">
                  {{ scope.row.type == null? '暂无' :scope.row.type ==3 ?'研究力': scope.row.type ==4 ?'技术谈':'' }}
              </template>
            </el-table-column>
            <el-table-column prop="type_id" label="类型" align="center" >
              <template slot-scope="scope">
                  {{ scope.row.type_id == 1? '早知道' :'最强音' }}
              </template>
            </el-table-column>
            <el-table-column prop="article_addtime" label="创建日期" :formatter="this.$common.timestampToTime" width="150" sortable>
            </el-table-column>
            ...
            <el-table-column label="操作" fixed="right" min-width="230">
                <template slot-scope="scope">
                    ...
                    <a href="#" style="color: #00D1B2" @click="articleEdit(scope.row)"><el-button size="mini" >编辑</el-button></a>
        ...
        <el-dialog  :title="formEditTitle" :visible.sync="dialogEdittVisible" width="700px" @close="closeDialog('formEdit')">
            <el-form :label-position="labelPosition" :label-width="labelWidth" :rules="rulesEdit" :disabled="formEditDisabled" :inline="true" ref="formEdit" :model="formEdit" class="demo-form-inline">
                <el-form-item label="微信平台" prop="wx_id">
                    <el-input v-model="formEdit.wx_id" placeholder="微信平台" disabled=""></el-input>
                </el-form-item>
...
export default {
    ...
    methods: {
        ...
        handleDelete(index, rowData) {
            ...
            this.$confirm('此操作将永久删除该文章, 是否继续?', '提示', {
                ...
                type: 'warning'
                    ...
                    apis.msgApi.articleDelete({id:id})
                    .then((data)=>{
                      this.$message({message: '执行成功',type: "success"});
                      this.onSearch();
                    })
        ...
        // 修改消息
        articleEdit(row){
          this.$router.push({ name: 'article_edit', query:row})
        },
