<template>
    <div class="container messageboard">
        ...
        <el-table :data="tableData" v-loading="listLoading"  border stripe style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="type_id" label="id" width="50" align="center" sortable>
                 <template slot-scope="scope">
                    <a href="javacript:;" style="color: #00D1B2" @click="openDetail(scope.row)">{{ scope.row.type_id}}</a>
                </template>
            </el-table-column>
            <el-table-column prop="type_name" label="用户类型" align="center" width="100">
            </el-table-column>
        ...
        <el-dialog  :title="formEditTitle" :visible.sync="dialogEdittVisible" width="700px" @close="closeDialog('formEdit')">
            <el-form :label-position="labelPosition" :label-width="labelWidth" :rules="rulesEdit" :disabled="formEditDisabled" :inline="true" ref="formEdit" :model="formEdit" class="demo-form-inline">
                <el-form-item label="用户类型" prop="type_name">
                    <el-input v-model="formEdit.type_name" placeholder="用户类型"></el-input>
                </el-form-item>
            </el-form>
...
export default {
    ...
    methods: {
        ...
        save() {
            this.$refs["formEdit"].validate(valid => {
                if(valid){
                    ...
                    apis.msgApi.userTyleAdd(param)
                    .then((data)=>{
                        if(data&&data.data){
                            var json=data.data;
                             if(json&&data.data.msg=="ok"){
        ...
        handleDelete(index, rowData) {
            ...
            var type_id=rowData.type_id;
            this.$confirm('此操作将永久删除, 是否继续?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                    apis.msgApi.userTyleDelete({type_id:type_id})
                    .then((data)=>{
                      console.log(data)
                        this.$message({message: '执行成功',type: "success"});
                        this.onSearch();
