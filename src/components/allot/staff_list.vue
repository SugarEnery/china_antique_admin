<template>
    <div class="container messageboard">
        ...
        <el-form :label-position="labelPosition" :label-width="labelWidth" :inline="true" ref="formSearch" :model="formSearch" class="demo-form-inline">
            <el-form-item label="账号归属" prop="admin_names">
                <el-input v-model="formSearch.admin_names" placeholder="搜索账号归属人名字"></el-input>
            </el-form-item>
            <el-form-item label="搜索账号" prop="admin_name">
                <el-input v-model="formSearch.admin_name" placeholder="搜索账号名"></el-input>
            </el-form-item>
        ...
        <el-table :data="tableData" v-loading="listLoading"  border stripe style="width: 100%" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="admin_id" label="账号id" width="50" align="center" sortable>
                 <template slot-scope="scope">
                    <a href="javacript:;" style="color: #00D1B2" @click="openDetail(scope.row)">{{ scope.row.admin_id}}</a>
                </template>
            </el-table-column>
            <el-table-column prop="admin_name" label="账号" align="center" >
            </el-table-column>
            <el-table-column prop="admin_names" label="员工" align="center" >
              <template slot-scope="scope">
                {{ scope.row.admin_names == null? '暂无' :scope.row.admin_names }}
              </template>
            </el-table-column>
            <el-table-column prop="admin_fid" label="父身份" align="center">
            </el-table-column>
            <el-table-column prop="admin_name" label="账号名" align="center" >
            </el-table-column>
            <!-- <el-table-column prop="depart_id" label="id" align="center" >
            </el-table-column> -->
            <el-table-column prop="depart_name" label="部门" align="center" >
            </el-table-column>
            <el-table-column label="操作" fixed="right" min-width="230">
                <template slot-scope="scope">
                    <a href="#" style="color: #00D1B2" @click="allotList(scope.row)"><el-button size="mini" >账号信息</el-button></a>
...
export default {
    ...
    methods: {
                    ...
                    function getUserRules(data){//创建方法
                      ...
                      if(data){//判断是否传进有效数组
                        for(let i=0;i<data.length;i++){//循环传进来数组
                          ...
                          if(data[i].branch !==null ){//判断当前项下面是否有source
                            cs[i].depart_id=data[i].branch.id//user_id用户的id
                            cs[i].depart_name=data[i].branch.depart_name//
        ...
        // 账号信息
        allotList(row){
            this.$router.push({ name: 'allot_list', query:row})
        }
